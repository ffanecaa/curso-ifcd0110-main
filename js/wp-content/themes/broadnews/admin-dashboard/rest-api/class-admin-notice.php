...
class AdminNotice {
    ...
    public function __construct() {
        ...
        if ( get_option( $this->dismiss_notice_key ) !== 'yes' ) {
			add_action( 'admin_notices', [ $this, 'broadnews_admin_notice' ], 0 );
			add_action( 'wp_ajax_aft_notice_dismiss', [ $this, 'broadnews_notice_dismiss' ] );
		}
	
    }

    function broadnews_admin_notice(){
        ...
        $this->broadnews_dashboard_notice_content();
        echo '</div>';

       
    }

    function broadnews_dashboard_notice_content(){
		if ( file_exists( WP_PLUGIN_DIR . '/af-companion/af-companion.php' ) ) {
			if(is_plugin_active('af-companion/af-companion.php')){
				$af_companion_title = __( 'Get Starter Sites', 'broadnews' );
				$af_companion_url = site_url( ).'/wp-admin/admin.php?page=af-companion';
			}else{
            $af_companion_title = __( 'Activate AF Companion', 'broadnews' );
            ...
            $af_companion_title = __( 'Install AF Companion', 'broadnews' );
        ...
        $notice_header = sprintf(
			...
			esc_html__( 'Howdy!', 'broadnews' ),
			sprintf(
				esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'broadnews' ),
				$this->theme_name
			)
		);
		...
		$demo_link = "https://afthemes.com/products/broadnews/";
		$notice_starter_msg =sprintf(
			...
			__( 'General info', 'broadnews' ),
			esc_html__( 'All of the features provided by the theme are now ready to use; Here, we have gathered all of the essential details and helpful links for you and your better experience with the theme. Once again, Thanks for using our theme!', 'broadnews' ),
			$af_companion_url,
			$af_companion_title,
			esc_url($demo_link),
			esc_html__('Demos/product','broadnews'),
			esc_url( admin_url() ."themes.php?page=".$this->page_slug ),
			esc_html__('Theme dashboard','broadnews')
		...
		$notice_external_msg =sprintf(
			...
			<div class="aft-documentation-links">
				<div>
					<a href="https://docs.afthemes.com/broadnews/" target="_blank"><span aria-hidden="true" class="dashicons dashicons-external"></span>%3$s</a>
			...
			__( 'Documentation', 'broadnews' ),
			esc_html__( 'Please check our full documentation for detailed information on how to setup and customize the theme.', 'broadnews' ),
			esc_html__( 'Docs', 'broadnews' ),
			esc_html__( 'Videos', 'broadnews' ),
			esc_html__( 'Blog', 'broadnews' ),
			esc_html__( 'Return to your dashboard', 'broadnews' )
    ...
    public function broadnews_notice_dismiss() {
